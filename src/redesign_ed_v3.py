"""Redesigned ED Figs 1-4: diverse panels, wide layout, generous spacing."""
import os
import numpy as np
import pandas as pd
import matplotlib
...
def ed1():
    """Estimator bias: heatmap + violin + area + 3D surface style."""
    ...
    # a: heatmap — bias ratio (noise × disturbance)
    ...
    ax1.set_xlabel("disturbance (m s$^{-1}$)"); ax1.set_ylabel("noise (m s$^{-1}$)")
    for ni in range(3):
        for di in range(6):
    ...
    # b: violin — distribution by gain_true at σ=0.15
    ...
    # c: area — IQR width vs disturbance
    ...
    # d: dumbbell — bias at threshold by gain
    ...
    # e: hexbin — bias vs window × disturbance
...
def ed2():
    """Controller validation: scatter + heatmap + dumbbell + violin."""
    ...
    # a: scatter with trend — chain gain
    ...
    # b: heatmap — speed at 0%
    ax2 = fig.add_subplot(gs[0, 1])
    vehicles = [5,15,25,35,45,55]
    tr0 = traces[(traces.pen==0)&(traces.veh.isin(vehicles))]
    ...
    # c: heatmap — speed at 50%
    ax3 = fig.add_subplot(gs[0, 2])
    tr50 = traces[(traces.pen==50)&(traces.veh.isin(vehicles))]
    ...
    # d: dumbbell — speed std by position
    ...
    ax4.set_xlabel("vehicle position"); ax4.set_ylabel("speed s.d.")
    ax4.legend()
    L(ax4, "d")

    # e: area — energy distribution
    ax5 = fig.add_subplot(gs[1, 1])
    from scipy.stats import gaussian_kde
    e0 = energy[energy.pen==0].e_kwh.clip(0,50)
    e50 = energy[energy.pen==50].e_kwh.clip(0,50)
    x = np.linspace(0, 50, 100)
    kde0 = gaussian_kde(e0)(x); kde50 = gaussian_kde(e50)(x)
    ax5.fill_between(x, kde0, alpha=0.4, color=HC, label="0%")
    ax5.fill_between(x, kde50, alpha=0.4, color=AVC, label="50%")
    ax5.set_xlabel("kWh/100km"); ax5.set_ylabel("density")
    ax5.legend()
    L(ax5, "e")

    # f: bar — cumulative gain at position 8
...
def ed3():
    """Sensitivity: heatmap + violin + dumbbell + box."""
    ...
    # a: heatmap — savings (jerk_cap × headway) at anticipation=0.3, pen=50%
    ax1 = fig.add_subplot(gs[0, :2])
    sub50 = df[df.n_av==30]
    ...
    # b: violin — savings distribution at each penetration
    ax2 = fig.add_subplot(gs[0, 2])
    data = []; labels = []
    ...
    # c: dumbbell — savings at 50% by jerk cap (low to high)
    ax3 = fig.add_subplot(gs[1, 0])
    ...
    ax3.text(0.02, 0.95, "blue: no anticip.", transform=ax3.transAxes,
             fontsize=5.5, color=HC)
    ax3.text(0.02, 0.88, "red: full anticip.", transform=ax3.transAxes,
             fontsize=5.5, color=AVC)
    L(ax3, "c")

    # d: box — savings by anticipation gain
    ax4 = fig.add_subplot(gs[1, 1])
    data_d = []
    for tc in [0.0,0.3,0.55]:
        data_d.append(sub50[sub50.t_close==tc].sav.values)
    ...
    ax4.set_xlabel("anticipation gain"); ax4.set_ylabel("saving at 50% (%)")
    L(ax4, "d")

    # e: hexbin — savings vs penetration
    ax5 = fig.add_subplot(gs[1, 2])
...
def ed4():
    """Perception validation: heatmap + ridgeline + scatter + bar."""
    ...
    # a: heatmap — median jerk by speed bin × source
    ax1 = fig.add_subplot(gs[0, :2])
    ...
    ax1.set_xlabel("speed (m s$^{-1}$)"); ax1.set_ylabel("median 95th p. jerk")
    for si in range(3):
        for bi in range(4):
    ...
    # b: ridgeline — jerk distribution by speed (human vs AV)
    ax2 = fig.add_subplot(gs[0, 2])
    from scipy.stats import gaussian_kde
    np.random.seed(0)
    ...
    ax2.set_yticks([]); ax2.set_xlabel("95th p. jerk (m s$^{-3}$)")
    L(ax2, "b")

    # c: scatter — speed vs jerk
    ax3 = fig.add_subplot(gs[1, 0])
    ...
    # d: dumbbell — key metric medians across sources
    ax4 = fig.add_subplot(gs[1, 1])
    for mi,(m,lab) in enumerate([("p95_abs_jerk","jerk"),("frac_aggr","aggr. share")]):
    ...
    ax4.set_xlabel("normalized to perceived human")
    L(ax4, "d")

    # e: area — aggressive share ECDF (stacked style)
    ax5 = fig.add_subplot(gs[1, 2])
    for s,c,lab in colors:
...
if __name__ == "__main__":
    ed1(); print("ED1: heatmap+violin+area+dumbbell+hexbin ✓")
    ed2(); print("ED2: scatter+heatmap+heatmap+dumbbell+area+bar ✓")
    ed3(); print("ED3: heatmap+violin+dumbbell+box+hexbin ✓")
    ed4(); print("ED4: heatmap+ridgeline+scatter+dumbbell+area ✓")
